...
**프로젝트 목록·RSVP 페이지 모바일 UI 개편**: ① /edit 행을 카드형으로 — 자주 쓰는 순서대로 큰 버튼 3개(RSVP 응답 강조색 채움 · 미리보기 · 편집, h-10 grid-cols-3), 거의 안 쓰는 이름 변경·복제·보관·삭제는 ⋯ 메뉴(섹션 목록 메뉴와 같은 패턴, ul overflow-hidden 제거로 클립 방지). ② 삭제는 **프로젝트 이름을 그대로 입력해야** 영구 삭제 버튼이 활성화(오터치 방어 한 단계 추가). ③ RSVP 응답 페이지: 헤더 두 줄 접힘(제목 truncate + 버튼 줄), 집계 grid 2열(sm:4열), 검색 입력 전폭(sm:w-64), 표는 모바일에서 구분·식사·제출 열을 숨기고(hidden sm:table-cell) 행 펼침 상세에 구분·식사 추가 — 상세가 전체 기록의 단일 소스. phase6 e2e를 메뉴 경유 + 이름 입력 삭제 흐름으로 갱신(잘못된 이름은 비활성 유지 검증 포함) — **로컬 Supabase 중단으로 e2e 미실행**.

**프로젝트 목록에서 RSVP 응답 바로가기**: /edit 각 프로젝트 행에 'RSVP 응답' 링크(→ /editor/{id}/rsvp) — 응답 확인은 편집만큼 잦아 편집기를 거치지 않는다. 보관된 프로젝트에는 편집과 같이 숨김. phase6 e2e에 링크 href·보관 시 숨김 검증 추가(로컬 Supabase 중단으로 미실행).
