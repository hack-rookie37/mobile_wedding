...
test("검토 → 일부 적용 → undo: 제안은 바로 적용되지 않고, 적용은 undo 1스텝", async ({ page }) => {
  ...
  // asset 라이브러리 로딩 후 hero 사진이 나타난다 (photoArch)
  const heroImages = canvas.locator("section").first().locator("img");
  await expect(heroImages.first()).toBeVisible();
  const heroImageCount = await heroImages.count();

  await requestProposal(page, EXAMPLE);

  // 변경 예정 목록: hero 레이아웃·여백, 갤러리 레이아웃, 테마 — 전후 비교 표기
  const changes = aiDialog(page).locator("[data-ai-change]");
  await expect(changes).toHaveCount(4);
  await expect(changes.nth(0)).toContainText("메인 — 레이아웃 변경");
  await expect(changes.nth(0)).toContainText("아치 사진");
  await expect(changes.nth(0)).toContainText("텍스트만");
  ...
  // 적용 결과: hero는 텍스트만(사진 없음), 테마는 그대로
  await expect(canvas).toHaveAttribute("data-canvas-theme", "warm-editorial");
  await expect(canvas.locator("section").first().locator("img")).toHaveCount(0);

  // undo 1번으로 전체(batch)가 되돌아간다
  await page.getByRole("button", { name: "실행 취소" }).click();
  await expect(heroImages).toHaveCount(heroImageCount);
});
