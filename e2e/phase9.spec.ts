...
test("게스트: 제출 → 성공 상태 → 소프트 가드 → 수정 제출, 동의 없이는 제출 불가", async ({
  ...
  await expect(guest.locator("[data-rsvp-form]")).toBeVisible();
...
test("발행 전 미리보기·비공개 미리보기의 RSVP 폼은 제출 불가 상태다", async ({ page, browser }) => {
  ...
  // 편집기 미리보기(인터랙션 모드)에서도 제출 버튼은 비활성 + 안내 문구
  await page.getByRole("button", { name: "인터랙션" }).click();
  const canvas = page.locator("[data-invitation-root]");
  await expect(canvas.getByRole("button", { name: "참석 의사 전달하기" })).toBeDisabled();
  await expect(canvas.getByText("게스트는 발행된 청첩장에서 제출할 수 있습니다.")).toBeVisible();
