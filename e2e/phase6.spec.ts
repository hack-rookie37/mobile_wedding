...
test("프로젝트: 생성·이름 변경·복제·보관·삭제와 마지막 수정 시간", async ({ page }) => {
  ...
  // 보관 해제
  await copyRow.getByRole("button", { name: "보관 해제" }).click();
  await expect(page.getByRole("heading", { name: "보관함" })).toHaveCount(0);

  // 삭제 (확인 단계)
  await copyRow.getByRole("button", { name: "삭제", exact: true }).click();
  await expect(copyRow.getByText("사진·기록까지 모두 삭제됩니다.")).toBeVisible();
  await copyRow.getByRole("button", { name: "영구 삭제" }).click();
