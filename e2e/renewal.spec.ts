...
test("메인: 사진 위 문구의 등장 효과를 고르면 그 방식대로 그려진다", async ({ page }) => {
  ...
  // 펜으로 쓰듯 — 글자가 아니라 '줄'마다, 창·잉크 두 상자가 transform으로 서로 상쇄되며
  // 쓸고 지나간다 (clip-path는 모바일에서 메인 스레드를 태워 끊겼다, ADR-044)
  await inspector(page).getByLabel("사진 위 문구").fill("우리\n결혼합니다");
  await effect.selectOption("writing");
  await expect(overlay.locator("[data-canvas-anim]")).toHaveCount(4); // 두 줄 × (창 + 잉크)
  const lineAnim = await overlay
    .locator("[data-canvas-anim]")
    .first()
    .evaluate((el) => getComputedStyle(el).animationName);
  expect(lineAnim).toBe("canvas-write-window");
  // 둘째 줄(창)은 첫 줄을 다 쓴 뒤에 시작한다 — DOM 순서: 창1, 잉크1, 창2, 잉크2
