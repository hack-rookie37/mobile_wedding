...
test("메인: 사진 위 문구의 등장 효과를 고르면 그 방식대로 그려진다", async ({ page }) => {
  ...
  // 펜으로 쓰듯 — 글자가 아니라 '줄'마다 하나씩, clip-path로 쓸고 지나간다
  await inspector(page).getByLabel("사진 위 문구").fill("우리\n결혼합니다");
  await effect.selectOption("writing");
  await expect(overlay.locator("[data-canvas-anim]")).toHaveCount(2);
  const lineAnim = await overlay
    .locator("[data-canvas-anim]")
    .first()
    .evaluate((el) => getComputedStyle(el).animationName);
  expect(lineAnim).toBe("canvas-write-in");
  // 둘째 줄은 첫 줄을 다 쓴 뒤에 시작한다
