...
test("발행 수명주기: slug 검증 → publish → draft 수정 미반영 → republish 반영 → unpublish", async ({
  ...
  // 준비 게이트(ADR-049)가 폰트·이미지 로드 후 반드시 풀린다 — 안 풀리면 효과가 영영
  // 멈춘 채라 치명적이다. SSR 인라인 스크립트가 실행됐다는 증거이기도 하다.
  await expect(anonPage.locator("[data-invitation-root]")).not.toHaveAttribute(
    "data-entrance-hold",
  );
...
test("공개 응답에 private 데이터가 없고, social metadata·공유(복사 fallback)가 동작한다", async ({
  ...
  // 공유: 떠 있는 공유 버튼 — 최상단에서는 숨어 있다가 스크롤할 때 떠오른다 (ADR-056)
  const floatingShare = anonPage.locator("[data-floating-share]");
  await expect(floatingShare).toBeHidden(); // 첫 화면(메인 사진)을 가리지 않는다
  await anonPage.mouse.wheel(0, 600);
  await expect(floatingShare).toBeVisible();
  // 스크롤이 멈추면 1.8초 뒤 사라지므로, 느린 러너에서는 다시 굴려 띄운 뒤 누른다
  await expect(async () => {
    await anonPage.mouse.wheel(0, 60);
    await floatingShare.getByRole("button", { name: "청첩장 공유하기" }).click({ timeout: 1_000 });
  }).toPass();
  await floatingShare.getByRole("button", { name: "링크 복사" }).click();
  await expect(floatingShare.getByText("복사되었습니다")).toBeVisible();
